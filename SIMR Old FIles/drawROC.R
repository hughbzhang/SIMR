roc <- function(swap){
  ...
  print(FP)
  print(TP)
  if(swap==1){ lines(FP,TP,type = 's',col = 'red',lwd = 5);}
  if(swap==2){ lines(FP,TP,type = 's',col = 'blue',lwd = 5);}
  if(swap==3){ lines(FP,TP,type = 's',col = 'green',lwd = 5);}
  if(swap==4){ lines(FP,TP,type = 's',col = 'orange',lwd = 5);}
  return(rbind(FP,TP))
}

set.seed(7)
plot(0:1,0:1,type = 'l',col = 'purple',main = 'ROC Curve for All Models',xlab = 'False Positive Rate',ylab = 'True Positive Rate',lwd = 5)
legend('bottomright', c('Combined','Imaging','Clinical','Jain','Random'),lty=1,lwd = 2, col=c('red', 'blue', 'green','orange','purple'), bty='n', cex=1)
roc(1)
roc(2)
roc(3)
roc(4)