...
for (i in 1:10){
  print(i)
  cur = sample(26)
  test = norm[norm$V66 %in% cur[1:2],]
  train = norm[norm$V66 %in% cur[3:26],]
  
  testx = as.matrix(test[,61:63])
  testy = as.integer(test$V65)
  trainx = as.matrix(train[,61:63])
  trainy = as.integer(train$V65)
  bestC = 30
  bestS = 0.001
